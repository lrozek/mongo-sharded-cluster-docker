...
#until mongos refreshes its state after config server replica set initialization and starts accepting incoming connections
while ! docker exec mongo-sharded-cluster-docker_mongos_1 mongo /router-init/router-init.js
do
	echo sleep
	sleep 1
done
